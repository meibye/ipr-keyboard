...
- **Purpose:** Bridges IrisPen USB scanner output to a paired device via Bluetooth HID keyboard emulation (target: Raspberry Pi).
...
- **Key Components:**
  ...
  - `src/ipr_keyboard/bluetooth/keyboard.py`: Wraps `/usr/local/bin/bt_kb_send` to send text as keyboard input. Supports BLE/uinput backends, selected via config and `/etc/ipr-keyboard/backend`.
...
- Use only the approved diagnostic scripts in `scripts/rpi-debug/` for remote troubleshooting (see [scripts/rpi-debug/README.md](../scripts/rpi-debug/README.md)).
- Do not run arbitrary commands; prefer plan-first, bounded diagnostic workflows.
- See `docs/copilot/DIAG_AGENT_PROMPT.md` for agent prompt structure and safe recovery ladder.
