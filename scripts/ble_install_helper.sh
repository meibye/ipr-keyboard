...
class HidControlPointCharacteristic(Characteristic):
    ...
    @dbus.service.method(GATT_CHRC_IFACE, in_signature="ay")
    def WriteValue(self, value):
        print(f"[ble] HID Control Point write: {list(value)}")
...
class HidInputReportCharacteristic(Characteristic):
    ...
    def StartNotify(self):
        if self.notifying:
            return
        print("[ble] InputReport StartNotify")
        self.notifying = True
    ...
    def StopNotify(self):
        if not self.notifying:
            return
        print("[ble] InputReport StopNotify")
        self.notifying = False
...
class BleHidServer:
    def __init__(self):
        ...
        print(f"[ble] Using adapter: {self.adapter_path}")
...
def fifo_worker(hid: BleHidServer) -> None:
    ...
    print(f"[ble] FIFO ready at {FIFO_PATH}")
    while True:
        try:
            with open(FIFO_PATH, "r", encoding="utf-8") as fifo:
                for line in fifo:
            ...
            print(f"[ble] FIFO worker error: {exc}")
            time.sleep(1.0)
...
def main() -> None:
    print("[ble] Starting BLE HID daemon...")
    dbus.mainloop.glib.DBusGMainLoop(set_as_default=True)
    server = BleHidServer()
    t = threading.Thread(target=fifo_worker, args=(server,), daemon=True)
    t.start()
    print("[ble] BLE HID ready. Waiting for connections and FIFO input...")
    loop = GLib.MainLoop()
    try:
        loop.run()
    except KeyboardInterrupt:
