...
def test_config_api(monkeypatch, tmp_path):
    ...
    save_json(cfg_file, {"IrisPenFolders": ["/tmp/usb"]})
    ...
    data = res.get_json()
    assert data["IrisPenFolders"] == ["/tmp/usb"]
...
def test_update_config_partial(flask_client):
    ...
    original_folders = initial["IrisPenFolders"]
    ...
    assert updated["IrisPenFolders"] == original_folders
